...
public class PlayerController : MobController
{
    ...
    protected virtual IInteractable GetSelectedEntity()
    {
        ...
        foreach (Collider collider in colliderBuffers)
        {
            ...
            if (!(collider.GetComponentInParent<Entity>() is IInteractable interactable))
                continue;
            if (!interactable.Selectable || !interactable.CanBeUsedBy(Possessed))
                continue;
    ...
    private IInteractable GetSelectedEntity_Inefficient()
    {
        ...
        Vector3 mobPos = Possessed.transform.position;
        float minDist = selectionDistance;
        foreach (Entity entity in Game.Entities.Where((Entity e) => e is IInteractable))
        {
            ...
            minDist = distance;
            result = entity as IInteractable;
            Debug.Log(distance);
        }
